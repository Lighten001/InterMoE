...
from evaluators.evaluator_interhuman import (
...
def evaluate_matching_score(motion_loaders, file):
    ...
    for motion_loader_name, motion_loader in motion_loaders.items():
        ...
        print(
            f"---> [{motion_loader_name}] MM Distance: {mm_dist:.4f}",
            file=file,
            flush=True,
        )
...
def evaluation(log_file, is_vae=False):
...
if __name__ == "__main__":
    ...
    params = parse_args()
    if params.cfg != None:
        cfg = get_config(params.cfg)
        data_cfg = cfg.dataset.interhuman_test
        cfg_path_list = [params.cfg]
    else:
        raise KeyError
    ...
    checkpoint = torch.load(
        mdoel_cfg.CHECKPOINT, map_location=torch.device("cpu"), weights_only=False
    )
    for k in list(checkpoint["state_dict"].keys()):
        # if "model" in k:
        if k.startswith("model."):
            checkpoint["state_dict"][k.replace("model.", "", 1)] = checkpoint[
                "state_dict"
            ].pop(k)
    ...
    eval_motion_loaders[mdoel_cfg.NAME] = lambda: get_motion_loader(
        batch_size, model, gt_dataset, device, mm_num_samples, mm_num_repeats
    )
    ...
    save_dir = pjoin(cfg.GENERAL.CHECKPOINT, cfg.GENERAL.EXP_NAME)
    epoch = checkpoint["epoch"]
    global_step = checkpoint["global_step"]
    log_file = pjoin(
        save_dir,
        f"eval-b-cfg{mdoel_cfg.CFG_WEIGHT}-e{epoch}-s{global_step}-{datetime.now().strftime('%y%m%d-%H%M%S')}.log",
    )
    evaluation(log_file, is_vae=(getattr(model, "_stage", None) == "vae"))
