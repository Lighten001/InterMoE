        ...
        for root, dirs, files in os.walk(pjoin(opt.DATA_ROOT, "motions_processed")):
            for file in tqdm(files):
                if file.endswith(".npy") and "person1" in root:
                    ...
                    motion1_swap, motion2_swap = process_both_motions(
                        motion1_swap, motion2_swap
                    )

                    self.motion_list.extend(
                        [motion1, motion1_swap, motion2, motion2_swap]
                    )

        self.indices = self._create_indices()
        print(
            "Total number of motions {}, snippets {}".format(
                len(self.motion_list), len(self.indices)
            )
        )
    ...
    def _create_indices(self):
        ...
        for i, sample in enumerate(self.motion_list):
            for start_idx in range(
                0, len(sample) - self.opt.WINDOW_SIZE + 1, self.opt.WINDOW_STRIDE
            ):
