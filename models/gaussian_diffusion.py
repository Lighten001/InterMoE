...
from models.losses import InterLoss, GeometricLoss, LatentInterLoss
            ...
            if isinstance(model_output, tuple):
                ...
                if loss_stratgy_name == "Capacity_Pred":
                    (
                        ...
                        b_pred_c_list,
                        layer_idx_list,
                        CapacityPred_loss_weight,
                    ) = model_output[2:]
                    for layer_idx, a_ones, a_pred_c, b_ones, b_pred_c in zip(
                        layer_idx_list,
                        a_ones_list,
                        a_pred_c_list,
                        b_ones_list,
                        ...
                        terms[f"Capacity_Pred_loss_{layer_idx}_a"] = (
                            nn.BCEWithLogitsLoss()(a_pred_c, a_ones)
                            * CapacityPred_loss_weight
                        )
                        terms[f"Capacity_Pred_loss_{layer_idx}_b"] = (
                            nn.BCEWithLogitsLoss()(b_pred_c, b_ones)
                            * CapacityPred_loss_weight
                        )
                else:
...
class MotionDiffusion(GaussianDiffusion):
    ...
    def __init__(self, use_timesteps, motion_rep, **kwargs):
        ...
        self.normalize = (motion_rep == "global")
    ...
    def training_losses(self, model, mask, t_bar, cond_mask, *args, **kwargs):
        ...
        if self.normalize and target.shape[-1] == 262:
        ...
        if target.shape[-1] == 262:
            ...
            # for latent only mse
            losses = {}
            latent_inter_loss_manager = LatentInterLoss("l2", "latent_inter_loss")
            latent_inter_loss_manager.forward(items["pred"], items["target"], mask[...,0,:], timestep_mask)
            losses.update(latent_inter_loss_manager.losses)

        # NOTE: moe loss
        losses['capa'] = th.scalar_tensor(0)
        for key in items.keys():
            if key.startswith("Capacity_Pred_loss"):
                losses["capa"] = losses["capa"] + items[key]
        losses["total"] += losses["capa"]
