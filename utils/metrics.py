...
def calculate_activation_statistics(activations, emb_scale=6):
    """
    Params:
...
def calculate_diversity(activation, diversity_times, emb_scale=6, divide_by=2):
    ...
    dist = linalg.norm(
        (activation[first_indices] - activation[second_indices]) / divide_by, axis=1
    )
    return dist.mean()
