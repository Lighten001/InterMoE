...
def swap_left_right(data, n_joints):
    ...
    new_data = np.concatenate(
        [positions.reshape(T, -1), rotations.reshape(T, -1)], axis=-1
    )
    return new_data
...
def print_current_loss(
    start_time, niter_state, losses, epoch=None, inner_iter=None, lr=None
):
    ...
    if epoch is not None and lr is not None:
        print(
            "epoch: %3d niter:%6d inner_iter:%4d lr:%5f"
            % (epoch, niter_state, inner_iter, lr),
            end=" ",
        )
    elif epoch is not None:
        print(
            "epoch: %3d niter:%6d inner_iter:%4d" % (epoch, niter_state, inner_iter),
            end=" ",
        )
